...
class ProcessingThread(QThread):
    ...
    def __init__(self):
        super().__init__()
        self.running = False
        self.max_q_sz = config["CAMERA"]["PROCESS_MAX_QUEUE_SIZE"]
        self.frame_queue = Queue(maxsize=self.max_q_sz)
        self.mutex = QMutex()
    ...
    def add_frame(self, frame):
        self.mutex.lock()
        if self.frame_queue.qsize() < self.max_q_sz:
    ...
    def process_frame(self, frame):
        detecting_data = {}
        bbox = None
        cls_count = {k: 0 for k in config["DETECTION"]["CLASSES_DICT"].keys()}
